...
class RadioGroup extends Component {
  ...
  _renderInputButtons = () => {
    ...
    const radioBtns = buttons.map((btn) => {
      ...
      return (
        <div onClick={clickAction} key={id} className={style}>
          <input type='radio' name={input.name} id={id} autoComplete='off' value={btnVal} checked={selected} />
  ...
  render () {
    ...
    return (
      <div className={`${instanceContainerClass} ${styling['radioGroup-container']} ${disabled ? styling['disabled'] : ''}`}>
        {
          label
        ...
        <div className={`${styling['field-container']} ${isStacked ? '' : styling['side-by-side']}`}>
