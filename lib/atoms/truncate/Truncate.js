...
import defaultStyles from './truncate.module.scss'
...
class Truncate extends React.Component {
  ...
  _renderMore () {
    ...
    const viewAction = (showViewAction)
      ? <span className={this.props.styling['truncate-view-action']} onClick={this._setExpandedState}>{lessText}<ChevronIcon isFilled width={13} direction='up' fillColor='currentColor'/></span>
      : ''
    return (
      <div>
        <div>
  ...
  _renderLess () {
    ...
    const {showViewAction} = this.state
    const viewAction = (showViewAction)
      ? <span className={this.props.styling['truncate-view-action']} onClick={() => { this._setExpandedState(); this.props.onToggle() }}>{moreText}<ChevronIcon isFilled width={13} direction='down' fillColor='currentColor'/></span>
      : ''
    return (
      <div>
...
export default ThemeWrapper({defaultStyles})(Truncate)
